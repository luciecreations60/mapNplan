    ...
    <Card className="travel-tool-card local-time-card">
      <header className="travel-tool-card__header">
        <div>
          ...
          <h2>Local time</h2>
          <small>{locationLabel || 'Destination'}</small>
        </div>
        <span className="travel-tool-card__header-icon"><Icon name="clock" /></span>
      </header>

      {formatted ? (
        <div className="local-time-card__body">
          <strong>{formatted.time}</strong>
          <span>{formatted.date}</span>
          <small>{timezoneAbbreviation || timezone}</small>
        </div>
      ) : (
        <div className="travel-tool-error travel-tool-error--compact">
          <Icon name="clock" size={26} />
          <strong>Time zone unavailable</strong>
          <p>Load the destination weather to resolve its local time zone.</p>
        </div>
      )}
    </Card>
  );
