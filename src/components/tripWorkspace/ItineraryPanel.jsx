...
export function ItineraryPanel({ trip, onUpdate }) {
    ...
    const nextItinerary = existingDay
      ? trip.itinerary.map((day) => (
          day.id === existingDay.id
            ? {
                ...day,
                items: [...day.items, newActivity].sort((left, right) => left.time.localeCompare(right.time)),
              }
            : day
        ))
      : [
  ...
  function removeActivity(dayId, activityId) {
    const nextItinerary = trip.itinerary
      .map((day) => (
        day.id === dayId
          ? { ...day, items: day.items.filter((item) => item.id !== activityId) }
          : day
      ))
    ...
    <div className="workspace-section">
      <section className="workspace-section__heading">
        <div>
          ...
          <h2>Itinerary</h2>
          <p>Keep every stop, booking and useful detail in chronological order.</p>
        </div>
        <Button icon={isFormOpen ? 'close' : 'plus'} onClick={() => setFormOpen((value) => !value)}>
          {isFormOpen ? 'Close' : 'Add activity'}
        </Button>
      </section>
          ...
          <form className="workspace-form" onSubmit={submitActivity}>
            <div className="workspace-form__grid">
              ...
              <Field label="Title" className="workspace-form__wide">
                <input name="title" value={form.title} onChange={updateField} placeholder="Visit, reservation, transport…" required />
              </Field>
              ...
              <Field label={`Estimated cost (${trip.currency})`}>
              ...
              <Field label="Notes" className="workspace-form__full">
                <textarea name="notes" rows="3" value={form.notes} onChange={updateField} placeholder="Booking details, reminders or useful context" />
              </Field>
            </div>
            <div className="workspace-form__actions">
              <Button variant="ghost" onClick={() => setFormOpen(false)}>Cancel</Button>
              <Button type="submit" icon="plus">Add to itinerary</Button>
            </div>
          </form>
        </Card>
      )}
      ...
      {itinerary.length > 0 ? (
          ...
          {itinerary.map((day, dayIndex) => (
            <Card key={day.id} className="itinerary-day">
              <header className="itinerary-day__header">
                ...
                <div>
                  <h3>{formatWorkspaceDate(day.date)}</h3>
                  <p>{day.title || `${day.items.length} planned item${day.items.length > 1 ? 's' : ''}`}</p>
                </div>
              </header>
              <div className="itinerary-list">
                {day.items.map((item) => (
                  <article key={item.id} className="itinerary-item">
                    <time>{item.time || '—'}</time>
                    <span className={`itinerary-item__icon itinerary-item__icon--${item.type}`}>
        ...
        <WorkspaceEmpty
          ...
          copy="Start with the arrival, accommodation and one memorable activity."
