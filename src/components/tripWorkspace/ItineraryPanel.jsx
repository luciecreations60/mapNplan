...
export function ItineraryPanel({ trip, onUpdate }) {
  ...
  const [form, setForm] = useState(() => ({ ...EMPTY_FORM, date: getLastUsedItineraryDate(trip) }));
  ...
  function openCreateForm(date = getLastUsedItineraryDate(trip)) {
    ...
    setForm({ ...EMPTY_FORM, date: date || trip.startDate || '' });
    setFormOpen(true);
    scrollToForm();
  }

  function openEditForm(day, activity) {
    const duration = splitDuration(activity.durationMinutes || 0);
  ...
  function submitActivity(event) {
    ...
    const activity = {
      ...
      linkedReservationId: previousActivity?.linkedReservationId || null,
    };
  ...
  function removeActivity(dayId, activity) {
    ...
    onUpdate({ itinerary: removeActivityFromItinerary(trip.itinerary, dayId, activity.id) });
  }
  ...
  function createReservationFromActivity(day, activity) {
    ...
    const existing = trip.reservations.find((reservation) => reservation.sourceActivityId === activity.id);
    if (existing) {
      window.alert(t('itinerary.reservationAlreadyExists'));
      return;
    }
    const reservationId = createId('reservation');
    const durationMinutes = Math.max(0, Number(activity.durationMinutes) || 0);
    const end = addMinutesToDateTime(day.date, activity.time, durationMinutes);
    const reservation = {
      id: reservationId,
      type: reservationType,
      title: activity.title,
      provider: '', confirmationNumber: '',
      startDate: day.date, startTime: activity.time || '',
      endDate: end.date, endTime: end.time,
      location: activity.location || activity.departureLocation || '',
      status: 'pending', amount: Math.max(0, Number(activity.estimatedCost) || 0), url: '',
      latitude: activity.latitude ?? null, longitude: activity.longitude ?? null,
      notes: activity.notes || '', reminderMinutes: null, externalCalendarUid: '', comments: [],
      sourceActivityId: activity.id, createdAt: new Date().toISOString(),
    };
    onUpdate({
      reservations: [...trip.reservations, reservation],
      itinerary: trip.itinerary.map((storedDay) => storedDay.id === day.id
        ? { ...storedDay, items: storedDay.items.map((item) => item.id === activity.id ? { ...item, linkedReservationId: reservationId } : item) }
        : storedDay),
    });
    window.alert(t('itinerary.reservationCreated'));
  }
                ...
                {day.items.map((item, itemIndex) => (
                  <article key={item.id} className="itinerary-item">
                    ...
                    <div className="itinerary-item__content">
                      ...
                      {item.type === 'car' && item.transportMode && <p><Icon name="route" size={14} /> {t(`itinerary.transportModes.${item.transportMode}`)}</p>}
                      ...
                      {reservationTypeForActivity(item.type) && <button className="text-link itinerary-item__reservation-link" type="button" onClick={() => createReservationFromActivity(day, item)}><Icon name="receipt" size={15} /> {item.linkedReservationId ? t('itinerary.reservationLinked') : t('itinerary.createReservation')}</button>}
