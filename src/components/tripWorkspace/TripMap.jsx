...
export function TripMap({ points, onMapClick = null, selection = null }) {
  ...
  useEffect(() => {
    ...
    map.getContainer().classList.add('trip-map--selectable');
    return () => {
      map.off('click', handleClick);
      map.getContainer().classList.remove('trip-map--selectable');
    };
  }, [onMapClick]);
  ...
  useEffect(() => {
    ...
    points.forEach((point, index) => {
      ...
      marker.bindTooltip(`${index + 1}. ${point.title}`, { direction: 'top', offset: [0, -7] });
  ...
  useEffect(() => {
    ...
    L.circleMarker(coordinates, { radius: 11, weight: 3, color: '#111827', fillColor: '#f7b84b', fillOpacity: 0.95, dashArray: '4 3' })
      .addTo(layer)
      .bindTooltip(t('map.selectedPoint'));
