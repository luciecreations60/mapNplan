...
export function MapPanel({ trip, onOpenTab, onUpdate }) {
  ...
  const selectMapPoint = useCallback(async ({ latitude, longitude }) => {
    setSelection({ latitude, longitude }); setStatus('loading'); setNotice(null);
    setForm((current) => ({ ...current, date: current.date || getLastUsedItineraryDate(trip), title: '', location: `${latitude.toFixed(5)}, ${longitude.toFixed(5)}` }));
    try {
      const place = await geocodingService.reverse(latitude, longitude, { language });
      setForm((current) => ({ ...current, title: place?.name || place?.primaryLabel || t('map.selectedPlaceFallback'), location: place?.label || current.location }));
    } catch {
  ...
  function addSelectedPlace(event) {
    event.preventDefault(); if (!selection || !form.date || !form.title.trim()) return;
    const activity = { id: createId('activity'), time: form.time, type: form.type, title: form.title.trim(), location: form.location.trim(), latitude: selection.latitude, longitude: selection.longitude, departureLocation: '', departureLatitude: null, departureLongitude: null, transportMode: '', durationMinutes: 60, estimatedCost: 0, notes: '', reminderMinutes: null, externalCalendarUid: '', completedAt: null, comments: [], linkedReservationId: null };
    onUpdate({ itinerary: upsertActivityInItinerary(trip.itinerary, form.date, activity) });
    setNotice({ tone: 'success', title: t('map.activityAddedTitle'), text: t('map.activityAddedText', { name: activity.title }) }); setSelection(null); setStatus('idle');
  }

  return <div className="workspace-section map-planner">
    <section className="workspace-section__heading"><div><p className="eyebrow">{t('map.eyebrow')}</p><h2>{t('map.title')}</h2><p>{t('map.intro')}</p></div></section>
    ...
    <div className="map-workspace-grid">
      ...
      <Card className="map-place-list">
        {selection ? <form className="map-add-form" onSubmit={addSelectedPlace}>
          <header className="workspace-panel__header"><div><p className="eyebrow">{t('map.newActivity')}</p><h2>{status === 'loading' ? t('map.searchingPlace') : t('map.configureActivity')}</h2></div><button className="icon-button icon-button--small" type="button" aria-label={t('common.close')} onClick={() => setSelection(null)}><Icon name="close" size={16} /></button></header>
          ...
          <div className="map-add-form__grid">
            <label className="workspace-field"><span>{t('itinerary.date')}</span><select name="date" value={form.date} onChange={updateField} required>{dates.map((date, index) => <option key={date} value={date}>{t('itinerary.day', { count: index + 1 })} · {formatDate(date, locale)}</option>)}</select></label>
          ...
          <div className="workspace-form__actions"><Button variant="ghost" onClick={() => setSelection(null)}>{t('common.cancel')}</Button><Button type="submit" icon="plus" disabled={status === 'loading'}>{t('map.addToItinerary')}</Button></div>
        </form> : <>
          ...
          {points.length > 0 ? <div className="map-place-list__items">{points.map((point, index) => <article key={point.id} className="map-place-row"><span className="map-place-row__number">{index + 1}</span><div><small>{point.source === 'reservation' ? t('map.reservation') : point.source === 'destination' ? t('map.destination') : point.source === 'savedPlace' ? t('map.savedPlace') : t('map.itinerary')}</small><strong>{point.title}</strong><p><Icon name="pin" size={13} /> {point.subtitle || t('map.savedCoordinates')}</p></div></article>)}</div> : <section className="workspace-large-empty workspace-large-empty--compact"><span><Icon name="map" size={28} /></span><h3>{t('map.emptyTitle')}</h3><p>{t('map.emptyText')}</p><Button icon="plus" onClick={() => onOpenTab('itinerary')}>{t('map.addCoordinates')}</Button></section>}
        </>}
      </Card>
    </div>
  </div>;
}
function formatDate(date, locale) { return new Intl.DateTimeFormat(locale, { weekday: 'short', day: 'numeric', month: 'short' }).format(new Date(`${date}T12:00:00`)); }
