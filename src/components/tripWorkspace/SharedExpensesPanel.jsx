...
import { evaluateMoneyExpression, normalizeMoneyExpression } from '../../utils/moneyExpression.js';
import { buildTripDateRange } from '../../utils/itinerary.js';
import { getBudgetedItineraryActivities, getUnlinkedBudgetedActivities } from '../../utils/activityExpenses.js';
import {
...
export function SharedExpensesPanel({ trip, onUpdate }) {
  ...
  const budgetedActivities = useMemo(() => getBudgetedItineraryActivities(trip), [trip]);
  const suggestedActivityExpenses = useMemo(() => getUnlinkedBudgetedActivities(trip), [trip]);
  ...
  function openEditExpense(expense) {
    ...
    const linkedActivity = budgetedActivities.find((activity) => (
    ...
    setExpenseForm({
      ...
      notes: expense.notes || '',
      sourceActivityId: linkedActivity?.activityId || expense.sourceActivityId || '',
      sourceActivitySeriesId: linkedActivity?.seriesId || expense.sourceActivitySeriesId || null,
    });
    setExpenseModalOpen(true);
  }
  ...
  function updateExpenseField(event) {
    ...
    if (name === 'sourceActivityId') {
      ...
      setExpenseForm((current) => activity
        ? applyActivityToExpenseForm(current, activity, participants)
        : { ...current, sourceActivityId: '', sourceActivitySeriesId: null });
      return;
    }
  ...
  function submitExpense(event) {
    ...
    const payload = {
      ...
      sourceActivityId: expenseForm.sourceActivityId || null,
      sourceActivitySeriesId: expenseForm.sourceActivitySeriesId || null,
    };
  ...
  function submitSettlement(event) {
    event.preventDefault();
    const amount = Math.max(0, evaluateMoneyExpression(settlementForm.amount) ?? 0);
    if (
      amount <= 0
  ...
  return (
    <div className="workspace-section shared-expenses">
      ...
      {suggestedActivityExpenses.length > 0 && (
        <Card className="workspace-panel activity-expense-suggestions">
          <header className="workspace-panel__header">
            <div><p className="eyebrow">{t('sharedExpenses.activitySuggestionsEyebrow')}</p><h3>{t('sharedExpenses.activitySuggestionsTitle')}</h3></div>
            <span>{suggestedActivityExpenses.length}</span>
          </header>
          <p className="workspace-muted-copy">{t('sharedExpenses.activitySuggestionsText')}</p>
          <div className="activity-expense-suggestions__list">
            {suggestedActivityExpenses.map((activity) => (
              <article key={activity.activityId} className="activity-expense-suggestion">
                <span><Icon name="wallet" size={18} /></span>
                <div>
                  <strong>{activity.title}</strong>
                  <small>{activity.date ? formatDate(activity.date, locale) : ''}{activity.location ? ` · ${activity.location}` : ''}</small>
                </div>
                <strong>{formatCurrency(activity.amount, trip.currency, locale)}</strong>
                <Button size="small" variant="secondary" icon="plus" onClick={() => openNewExpense(activity)}>{t('sharedExpenses.useActivityBudget')}</Button>
              </article>
            ))}
          </div>
        </Card>
...
function ExpenseDialog({ isOpen, form, participants, currency, itineraryDates, budgetedActivities, locale, t, validationError, onClose, onFieldChange, onMoneyBlur, onToggleParticipant, onShareAmountChange, onShareAmountBlur, onSubmit }) {
  ...
  const expectedTotal = Math.max(0, evaluateMoneyExpression(form.amount) ?? 0);
        ...
        <div className="workspace-form__grid">
          {budgetedActivities.length > 0 && (
            <label className="workspace-field workspace-form__full activity-budget-picker">
              <span>{t('sharedExpenses.fromActivityBudget')}</span>
              <select name="sourceActivityId" value={form.sourceActivityId || ''} onChange={onFieldChange}>
                <option value="">{t('sharedExpenses.noActivityBudget')}</option>
                {budgetedActivities.map((activity) => <option key={activity.activityId} value={activity.activityId}>{activity.date ? `${formatDate(activity.date, locale)} · ` : ''}{activity.title} — {formatCurrency(activity.amount, currency, locale)}</option>)}
              </select>
              <small className="field-hint">{t('sharedExpenses.activityBudgetHint')}</small>
            </label>
          )}
...
function applyActivityToExpenseForm(form, activity, participants) {
  return {
    ...form,
    ...
    paidAmount: '',
    date: activity.date || form.date,
    splitBetweenIds: participants.map((participant) => participant.id),
    splitMode: 'equal',
    shareAmounts: Object.fromEntries(participants.map((participant) => [participant.id, ''])),
    sourceActivityId: activity.activityId,
    sourceActivitySeriesId: activity.seriesId || null,
  };
}
